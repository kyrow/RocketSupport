        ...
        <div class="card card-warning">
            ...
            <div class="card-body">
                @foreach ($supportsEnds as $end)
                    <div>
                        У сайта: <span class="text-bold">{{ $end->title }}</span> - До: <span
                            class="text-bold">{{ $end->support_end }}</span>
                    </div>
                @endforeach
            </div>
                                    ...
                                    <td>
                                        @if (!empty($support->whatsapp))
                                            <a href="https://wa.me/{{ $support->whatsapp }}"
                                                target="_blank">{{ $support->whatsapp }}</a>
                                        @else
                                            Номера нет
                                        @endif
                                    </td>
                                    <td style="word-wrap: break-word;max-width: 160px;">{{ $support->completed ?? '-' }}
                                    </td>
                                    <td data-label="Дата">{{ $support->support_end }}</td>
                                    <td>
                                        <a class="btn btn-success" href="{{ route('support.edit', $support->id) }}"><i
                                                class="fas fa-pen"></i></a>
                                        <form style="display: inline" action="{{ route('support.destroy', $support->id) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
