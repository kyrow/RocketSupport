...
@section('content')
    <div class="row">
        ...
        {{-- <div class="col-6">
            <a style="color: white;font-size:14px;text-decoration:underline" href="https://t.me/silero_voice_bot">Работа с
                данными также для удобства может происходит через телеграм бота</a>
        </div> --}}
                        ...
                        <tbody class="sites-table">
                            @foreach ($sites as $access)
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-success" href="{{ route('sites.edit', $access->site_id) }}"><i
                                                class="fas fa-pen"></i></a>
                                        <form style="display: inline"
                                            action="{{ route('sites.destroy', $access->site_id) }}" method="POST">
                                            @method('DELETE')
                                            @csrf
